...
public class UserRegisterDTO {
    ...
    private void validate() throws IllegalAccessException {
        ...
        if (!isEmailValid) {
            throw new IllegalArgumentException(EMAIL_NO_VALID_MESSAGE);
        }

        final boolean isPasswordValid = Pattern.matches(PASSWORD_VALIDATOR, this.password);

        if (!isPasswordValid) {
            throw new IllegalArgumentException(PASSWORD_NO_VALID_MESSAGE);
        }
